...
## Message delivery (producer `acks`)

* `acks=0`: The producer considers the records successfully delivered once it sends the records to the broker. This is basically “fire and forget.”
* `acks=1`: The producer waits for the lead broker to acknowledge that it has written the record to its log.
* `acks=all` (`-1`): The producer waits for an acknowledgment from the lead broker and from the in-sync replicas that they have successfully written the record to their logs.
